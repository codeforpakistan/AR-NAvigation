...
public class CustomShapeManager : MonoBehaviour {
    ...
    public void AddDestinationShape() {
        ...
        Debug.Log("lastInfo Px=" + lastInfo.px);
        Debug.Log("lastInfo Py=" + lastInfo.py);
        Debug.Log("lastInfo Pz=" + lastInfo.pz);
        Debug.Log("lastInfo Qx=" + lastInfo.qx);
        Debug.Log("lastInfo Qy=" + lastInfo.qy);
        Debug.Log("lastInfo Qz=" + lastInfo.qz);

        GameObject shape = ShapeFromInfo(lastInfo);
        shape.GetComponent<DiamondBehavior>().Activate(true);
        //destroy last shape
        DestroyImmediate (shapeObjList [shapeObjList.Count - 1],true);
